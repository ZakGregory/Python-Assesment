...
def two(input):
	...
	next_bert_index = lower_string.find("bert")

	if next_bert_index!=-1:
		no_bert_first=lower_string[next_bert_index+4:]
	next_bert_index = no_bert_first.find("bert")
	if next_bert_index!=-1:
		no_bert = no_bert_first[:next_bert_index]
...
def five(input):
	...
	for i in range(0, len(input_split)):
		if i%4 ==2 and input_split[i]=="False":
			name_list.append(input_split[i-2])
...
def six(input):
	...
	if ie!=-1 and input[ie-1]=="c":
		return False
	elif ie!=-1 and input[ie-1]!="c":
		return True
	if ei!=-1 and input[ei-1]=="c":
		return True
	elif ei!=-1 and input[ei-1]!="c":
		return False
...
def nine(inputString, char):
	smallString=inputString.replace(" ","")
	value= smallString.find(char)
	if value >=0:
		value+=1
	return value
...
def ten(string, int, char):
	...
	if int<len(lower_string) and lower_string[int-1]==char:
		return True
	else:
		return False
